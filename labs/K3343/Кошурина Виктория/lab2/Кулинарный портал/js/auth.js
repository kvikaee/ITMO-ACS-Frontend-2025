...
async function subscribe(authorId) {
    ...
    try {
        ...
        await fetch(`${API_BASE_URL}/subscriptions`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: user.id, authorId: authorIdStr })
        });

        // Обновляем массив подписок у пользователя
...
async function unsubscribe(authorId) {
    ...
    try {
        // Удаляем запись из subscriptions
        const response = await fetch(`${API_BASE_URL}/subscriptions?userId=${user.id}&authorId=${authorIdStr}`);
        const subs = await response.json();
        if (subs.length > 0) {
            await fetch(`${API_BASE_URL}/subscriptions/${subs[0].id}`, {
                method: 'DELETE'
            });
        }
